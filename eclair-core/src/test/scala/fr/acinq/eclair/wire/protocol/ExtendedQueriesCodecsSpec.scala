...
class ExtendedQueriesCodecsSpec extends AnyFunSuite {
  ...
  test("encode reply_channel_range (with optional timestamps)") {
    val replyChannelRange = ReplyChannelRange(
      ...
      1.toByte,
      EncodedShortChannelIds(CompressionAlgorithm.Uncompressed, List(ShortChannelId(142), ShortChannelId(15465), ShortChannelId(4564676))),
      Some(EncodedTimestamps(CompressionAlgorithm.ZlibDeflate, List(Timestamps(1, 1), Timestamps(2, 2), Timestamps(3, 3)))),
      None)
  ...
  test("encode reply_channel_range (with optional timestamps, checksums, and unknown data)") {
    val replyChannelRange = ReplyChannelRange(
      ...
      TlvStream(
        List(
          EncodedTimestamps(CompressionAlgorithm.ZlibDeflate, List(Timestamps(1, 1), Timestamps(2, 2), Timestamps(3, 3))),
